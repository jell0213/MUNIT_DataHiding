...
def Embed_mod(in_dir,out_dir,edge_dir,num,mod_num):
    embed_ratio = int(input("embed ratio = "))                                                                      #輸入嵌密率embed_ratio
    while embed_ratio < 0 or embed_ratio > 100 :                                                                    #簡易防呆   
        embed_ratio = int(input("embed ratio = "))        
        ...
        for row in range(image_embed.shape[0]):
            for col in range(image_embed.shape[1]):                
                if location_map[row][col]!= 1:                                                                      #pixel_num對100取餘數，如果小於嵌密率，就進行嵌密
                    if (pixel_num%100) < embed_ratio :                                                              #紅黑可嵌密
...
Embed_mod('./test','./test/embed','./test/input.png',20,3)
