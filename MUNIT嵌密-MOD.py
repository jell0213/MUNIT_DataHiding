...
def Embed_mod(in_dir,out_dir,edge_dir,num,mod_num):
    if not os.path.exists(out_dir) :                                                                               #建立路徑(資料夾)
        os.mkdir(out_dir)
    for i in range(num):
        if not os.path.exists(out_dir+"/output{:08d}".format(i)) :                                                 #建立路徑(子資料夾)
        ...
        io.imsave(out_dir+"/output{:08d}".format(i)+"/output{:08d}.png".format(i),image)                       #從in_dir複製原圖到cover資料夾(用來進行IQA比較)
        location_map=[]
        f = open(out_dir+"/output{:08d}".format(i)+"/output{:08d}_loaction map.txt".format(i),'w')           #紀錄location.txt
        ...
        for row in range(image_embed.shape[0]):
            for col in range(image_embed.shape[1]):
                if location_map[row][col]!= 1:                                                                              #紅黑可嵌密
                    ...
                    f.write(str(code))
        f.close()
        io.imsave(out_dir+"/output{:08d}".format(i)+"/output{:08d}_embed.png".format(i),image_embed)
...
def Mod(pixel,mod_num):                                                                                          #計算mod的嵌密結果
    ...
    if abs(p3[0]-pixel)<=abs(p3[1]-pixel) and abs(p3[0]-pixel)<=abs(p3[2]-pixel) :
        ...
        return p3[2],s
Embed_mod('./oneshoe/oneshoe1-1','./oneshoe1','./oneshoe/oneshoe-edge.png',20,5)                                #input路徑+output路徑+原框路徑+數量+mod值
Embed_mod('./oneshoe/oneshoe2-1','./oneshoe2','./oneshoe/oneshoe2-edge.png',20,5)                                #input路徑+output路徑+原框路徑+數量+mod值
Embed_mod('./oneshoe/oneshoe3-1','./oneshoe3','./oneshoe/oneshoe3-edge.png',20,5)                                #input路徑+output路徑+原框路徑+數量+mod值
