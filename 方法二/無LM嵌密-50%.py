        ...
        for row in range(image_embed.shape[0]):
            for col in range(image_embed.shape[1]):                
                if location_map[row][col]!= 1 :                                                                     #非白區可嵌密
                    if (pixel_num%2) < 1 :                                                                          #pixel_num對100取餘數，如果小於嵌密率，就進行嵌密
...
def Mod(pixel,mod_num):                                                                                             #計算mod的嵌密結果
    ...
    if pp<0 :
        ...
        pp=pp
    return pp,s
Embed_mod('D:\\108resercher\\====######RESEARCH######====\\GAN-research\\10000','D:\\108resercher\\====######RESEARCH######====\\GAN-research\\12.8\\50%MOD3',3,5000)
